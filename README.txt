...
The ``plone.recipe.varnish`` recipe creates a Varnish configuration file and creates
...
In order to use this recipe with varnish <= 2.0,
the ``varnish_version`` option must be set to 2.

It can be configured with any of these options:

``varnish_version``
    Varnish target version. Default is ``3``.
...
``cache-location``
    ...
    (e.g. ``parts/varnish/storage``).  Changing the default location can be
...
``bind``
    ...
    to ``127.0.0.1:8000``. The port will be used in the generated VHM URLs,
    unless ``zope2_vhm_port`` is set.

``config``
    Path for a Varnish VCL configuration to use. If you use this option
    you cannot use the ``backends``, ``zope2_vhm_map`` or ``verbose-headers``
    options .
...
``purge-hosts``
    Specifies hostnames or IP addresses for purge ACL. ``localhost`` and
    the backends are always allowed to purge.
    Additional allowed hosts are listed here.
...
``zope2_vhm_port``
    ...
    as haproxy. Defaults to ``bind`` port (see above).
