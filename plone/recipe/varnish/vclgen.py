...
class VclGenerator(object):
    ...
    def _directors(self):
        ...
        for director_cfg in self.cfg["directors"]:
            if "name" not in director_cfg:
                self._log_and_raise(
                    "director has no name {0}".format(director_cfg["type"])
                )

            if director_cfg["type"] not in DIRECTOR_TYPES:
                self._log_and_raise(
                    "director type {0} not supported.".format(director_cfg["type"])
                )
            director = dict()
    ...
    def _vhostings(self, directors):
        ...
        for idx, backend in enumerate(self.cfg["backends"]):
            ...
            if backend["url"][0] in "/:":
                ...
                vh[
                    "match"
                ] = 'req.http.host ~ "^{0}(:[0-9]+)?$" && ' 'req.url ~ "^/{1}"'.format(
                    hostname, path.lstrip(":/")
                )
            else:
                # set backend based on hostname
                vh["match"] = 'req.http.host ~ "^{0}(:[0-9]+)?$"'.format(backend["url"])
    ...
    def _purgehosts(self):
        ...
        purgehosts.update([_["host"] for _ in self.cfg["backends"] if "host" in _])
        return purgehosts
