...
meant for the latest LTS version of Varnish, version 6.0.x . Earlier major
releases of this recipe supported multiple Varnish versions. However
Varnish Software changed the release schedule to 6 month cycles where experimental
features are released to final versions. Now and then from the released Varnish

The different templates and matrix of options got so unwieldy to manage that the
recipe was stripped to only support 
